...
class WebSeparator:
    ...
    def __init__(
        ...
        self.feedback_tokens = feedback_tokens
    ...
    def run(self, iterations=300):
        ...
        for i, item in tqdm(enumerate(self.collection.find())):
            # ID = item['_id']
            url = item['Host']
            ...
            url_struct.form_labels_centroid_maintokens(
                ...
                save_feedback_tokens=self.feedback_tokens)
        ...
        print(colored(f'Average time on website : {round(spended_time / i, 2)}', 'cyan'))
        print(colored(f'Average ws by one second: {int(1 / (spended_time / i))}', 'cyan'))

        # import pandas as pd
        # out_df = pd.DataFrame()
        # out_df['URL'] = list(predicted_topics.keys())
        # out_df['Predicted_topics'] = list(predicted_topics.values())
        # print(f'Predicted DataFrame:\n{out_df}')



if __name__ == '__main__':
    web_separator = WebSeparator()
